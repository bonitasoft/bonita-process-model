...
- The release workflows skip the tests — `release.yml` passes `-DskipTestProject` (the gitflow
  plugin flag) and `publish.yml` passes `-DskipTests` — so a broken model-version invariant is not
  caught during the release itself, only on the next `Build` run.
