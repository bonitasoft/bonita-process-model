...
class DocumentGroovyScriptExpressionFactoryTest {

    @Test
    void should_create_an_expression_for_a_multiple_child_input() throws Exception {
        final DocumentGroovyScriptExpressionFactory scriptExpressionFactory = new DocumentGroovyScriptExpressionFactory();
        final ContractInputBuilder fileInput = aContractInput().withName("myFile").multiple()
        ...
        final Expression expression = scriptExpressionFactory
                .createMultipleDocumentInitialContentScriptExpression(fileInput.build());
    ...
    @Test
    void should_create_an_expression_for_a_multiple_root_input() throws Exception {
        final DocumentGroovyScriptExpressionFactory scriptExpressionFactory = new DocumentGroovyScriptExpressionFactory();
        final ContractInputBuilder fileInput = aContractInput().withName("myFile").multiple()
                .withType(ContractInputType.FILE);
    ...
    @Test
    void should_create_an_expression_for_for_single_child_input_contained_in_a_multiple_parent_input()
            throws Exception {
        final DocumentGroovyScriptExpressionFactory scriptExpressionFactory = new DocumentGroovyScriptExpressionFactory();
        final ContractInputBuilder fileInput = aContractInput().withName("myFile").withType(ContractInputType.FILE);
        aContractInput().withName("parent").withType(ContractInputType.COMPLEX).multiple().havingInput(fileInput)
                .build();

        final Expression expression = scriptExpressionFactory
                .createMultipleDocumentInitialContentScriptExpression(fileInput.build());

        ExpressionAssert.assertThat(expression).hasContent("parent.collect{it.myFile}.flatten()")
    ...
    @Test
    void should_create_an_expression_for_root_single_input() throws Exception {
        final DocumentGroovyScriptExpressionFactory scriptExpressionFactory = new DocumentGroovyScriptExpressionFactory();
        final ContractInputBuilder fileInput = aContractInput().withName("myFile").withType(ContractInputType.FILE);

        final Expression expression = scriptExpressionFactory
                .createSingleDocumentInitialContentScriptExpression(fileInput.build());
    ...
    @Test
    void should_create_an_expression_for_child_single_input_contained_in_a_single_input() throws Exception {
        final DocumentGroovyScriptExpressionFactory scriptExpressionFactory = new DocumentGroovyScriptExpressionFactory();
        final ContractInputBuilder fileInput = aContractInput().withName("myFile").withType(ContractInputType.FILE);
        aContractInput().withName("parent").withType(ContractInputType.COMPLEX).havingInput(fileInput).build();
        final Expression expression = scriptExpressionFactory
        ...
        ExpressionAssert.assertThat(expression).hasContent("parent.myFile")
