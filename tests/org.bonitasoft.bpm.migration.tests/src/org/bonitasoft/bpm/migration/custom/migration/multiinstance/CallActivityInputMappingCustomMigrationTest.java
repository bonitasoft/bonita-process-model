...
import static org.bonitasoft.bpm.migration.custom.migration.InstanceBuilder.aModel;
import static org.bonitasoft.bpm.migration.custom.migration.InstanceBuilder.aStringDataInstance;
import static org.bonitasoft.bpm.migration.custom.migration.InstanceBuilder.anInstance;
import static org.bonitasoft.bpm.migration.custom.migration.InstanceBuilder.clearModel;
...
public class CallActivityInputMappingCustomMigrationTest {
    ...
    public void should_migrateBefore_stores_data_instances_from_processSource_of_InputMappings() throws Exception {
        ...
        assertThat(callActivityInputMappingCustomMigration.getDataInstances()).containsKeys(inputMapping1.getUuid(),
                inputMapping2.getUuid());
    }
    ...
    public void should_migrateAfter_convert_data_instances_to_expression_in_processSource_of_InputMappings()
        ...
        assertThat((Instance) inputMapping1.get(ProcessPackage.Literals.INPUT_MAPPING__PROCESS_SOURCE)).isNotNull()
                .isInstanceOf(Instance.class);
        assertThat((Instance) inputMapping2.get(ProcessPackage.Literals.INPUT_MAPPING__PROCESS_SOURCE)).isNotNull()
                .isInstanceOf(Instance.class);

        assertThat(((Instance) inputMapping1.get(ProcessPackage.Literals.INPUT_MAPPING__PROCESS_SOURCE)).getType()
                .getEClass()).isEqualTo(
                        ExpressionPackage.Literals.EXPRESSION);
        assertThat(((Instance) inputMapping2.get(ProcessPackage.Literals.INPUT_MAPPING__PROCESS_SOURCE)).getType()
                .getEClass()).isEqualTo(
                        ExpressionPackage.Literals.EXPRESSION);

        //Data instances are deleted
        assertThat(callActivityInputMappingCustomMigration.getDataInstances().values()).hasSize(2).extracting("type")
