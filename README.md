...
|-----------------------------|------------------------------------------------------------------------------|-------------|---------------------------------|
| **version**                 | Version to release (required for Tycho builds)                               | none        | `9.0.4`                         |
...
| **previous-tag**            | Previous tag, used as starting point for release notes generation (required) | none        | `9.0.3`                         |
| **is-latest-release**       | Mark the GitHub Release as the latest release                                | `false`     | `true`                          |
...
When you run the workflow from any branch (develop or support/*), it runs three jobs in sequence:

1. **release** - runs `gitflow:release`:
...
2. **publish** - checks out the release tag and deploys the artifacts to Maven Central (`./mvnw deploy -Prelease`)
3. **github-release** - creates the GitHub Release for the tag, with release notes generated from `previous-tag` to
   the new tag

The `previous-tag` input is checked at the start of the **release** job, so a missing or mistyped tag fails the run
before anything is pushed.

If the **github-release** job fails for a transient reason (network, token), re-run only that job: the artifacts are
already on Maven Central and must not be deployed again. A re-run reuses the same inputs, so it cannot fix a wrong input.

Only one release can run at a time per branch. A second run waits for the first one to finish, but only one run can
wait: dispatching a third run cancels the one already waiting.

**Note**: The workflow does NOT create a release branch - it performs the release directly on the branch you selected.

### Release Notes

Release notes are generated by GitHub from merged pull requests, using the configuration in
[`.github/release.yml`](.github/release.yml) (picked up automatically by `gh release create --generate-notes`).
Pull requests are grouped into categories by label. Pull requests labelled `github_actions` (set automatically on
`ci/*` branches and on Dependabot GitHub Actions updates) or `ignore-for-release-notes` are excluded.
